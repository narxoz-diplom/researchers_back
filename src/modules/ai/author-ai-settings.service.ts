...
export class AuthorAiSettingsService {
  ...
  async upsertKey(
    ...
    const encryptedApiKey = this.encrypt(trimmed);
    const keyHint = this.buildKeyHint(trimmed);
  ...
  async getDecryptedKey(userId: string): Promise<string | null> {
    ...
    return this.decrypt(row.encryptedApiKey);
  }

  private buildKeyHint(apiKey: string): string {
